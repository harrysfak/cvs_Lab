# Σύστημα Επεξεργασίας Δεδομένων excel -> csv

Εφαρμογή σε Python για καθαρισμό, μορφοποίηση και παραγωγή τελικού CSV από αρχεία Excel, με ενδιάμεσα zero calibration blocks. Το README εξηγεί με σαφή σειρά τι κάνει η εφαρμογή από τη στιγμή που εισάγετε το Excel μέχρι το τελικό αποτέλεσμα και πώς μπορείτε να ρυθμίσετε κάθε βήμα.
...
1) **Εισαγωγή Excel**  
   ...
   - Υπάρχει και η επιλογή απο το παράθυρο διαλόγου και επιλογής αρχείων απο απο τον φάκελο εργασίας (__BASE__)
...
3) **Έλεγχος δεκαδικών & παράγωγες τιμές**  
   - Ελέγχονται τα δεκαδικά στις βασικές στήλες (2 ή 4 δεκαδικά). Υπολογίζονται TS/SNF/Fat/Protein/Lactose και FPD αντίστοιχα. (`modules/data_processor.py`)
...
- **Πιθανά σενάρια**:  
  ...
  - **Έλλειψη αρχείου zero.xlsx**: Κατεβαίνει αυτόματα από το Supabase URL του `config.py` ή βγάζει ERROR και ο χρήστης προσθέτει το zero.xslx στον φακελο για την συνέχεια της επεξεργασίας δεδομένων.  
...
- **Τοποθεσία**: Από προεπιλογή, το τελικό αρχείο γράφεται στο `FINAL_OUTPUT_PATH`. Το όνομα μπορεί να προσαρμοστεί μέσω των ρυθμίσεων (ΠΑΝΩ ΔΕΞΙΑ ΣΤΟ ΠΡΟΓΡΑΜΜΑ ~ ΧΡΕΙΑΖΕΤΑΙ ΕΠΑΝΑΚΙΝΗΣΗ ΤΟΥ ΠΡΟΓΡΑΜΜΑΤΟΣ ΓΙΑ ΕΝΗΜΕΡΩΣΗ ΤΩΝ ΑΛΛΑΓΩΝ) `config.py`.
